...
try {
    ...
    struct termios oldtiocom,newtiocom;                                         //
    ...
    /*
    c_iflag：輸入模式設定，控制終端輸入方式
    c_oflag：輸出模式設定，控制終端輸出方式
    c_cflag：控制模式設定，指定終端硬體控制信息
    c_lflag：本地模式設定，控制終端編輯功能
    */

    if (baudrate==115200) {                                               //是否使用奇偶校驗
    ...
    newtiocom.c_iflag=IGNPAR;                                                   //忽略輸入的同位錯誤（不進行奇偶校驗）
    newtiocom.c_oflag=0;
    newtiocom.c_lflag=0;
    newtiocom.c_cc[VMIN]=1;                                                     //讀取時的最小字元數（當緩衝區長度>1才能讀取）
    newtiocom.c_cc[VTIME]=0;                                                    //讀取時等待的最小時間

    tcflush(rs232fd,TCIFLUSH);                                                  //刷新寫入的數據,但是不讀
    tcsetattr(rs232fd,TCSANOW,&newtiocom);                                      //立即改變
